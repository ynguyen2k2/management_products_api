...
const PRODUCTION_COLLECTION_SCHEMA = Joi.object({
    ...
    description: Joi.string().default("Description of product").min(3).max(100).trim().strict(),
...
const createNew = async (data) => { 
    try {
        ...
        const result = await client.query(createNewQuery,[name, description, cover,slug])
...
const getDetails = async(id) => {
    // const result = await 
    try {
        const getDetailsQuery = `SELECT * FROM  products WHERE id = $1::text `;
        const client = await pool.connect()
